...
@pytest.mark.asyncio
async def test_restore_from_database(queue, mock_engine):
    """Test restoring queue from database on startup."""
    # Mock tasks in database
    queued_tasks = [
        IndexingTask(task_id="task1", paths=["/path1"], status="queued"),
        IndexingTask(task_id="task2", paths=["/path2"], status="queued")
    ]
    running_tasks = [
        IndexingTask(task_id="task3", paths=["/path3"], status="running", started_at=datetime.now())
    ]
    
    mock_engine.list_tasks_by_status.side_effect = lambda status: {
        "queued": queued_tasks,
    ...
    # Verify running tasks were reset to queued
    assert running_tasks[0].status == "queued"
    assert running_tasks[0].started_at is None
    mock_engine.update_indexing_task.assert_called_with(running_tasks[0])
    
    # Verify all tasks added to queue in correct order
...
async def test_queue_shutdown_waits_for_current_task():
    ...
    # Create real engine for this test
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        await queue.add_task(task)
        await asyncio.sleep(0.1)  # Let worker pick up task
