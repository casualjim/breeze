...
class BreezeInstaller:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "BREEZE_CONCURRENT_READERS": os.environ.get("BREEZE_CONCURRENT_READERS", "20"),
            "BREEZE_CONCURRENT_EMBEDDERS": os.environ.get("BREEZE_CONCURRENT_EMBEDDERS", "10"),
            "BREEZE_CONCURRENT_WRITERS": os.environ.get("BREEZE_CONCURRENT_WRITERS", "10"),
            "BREEZE_VOYAGE_CONCURRENT_REQUESTS": os.environ.get("BREEZE_VOYAGE_CONCURRENT_REQUESTS", "5"),
    ...
    def _detect_venv(self) -> Path:
        ...
        print("Please create or activate a virtual environment before running this script.")
        sys.exit(1)
    ...
    def check_requirements(self) -> bool:
        ...
        if any(provider in embedding_model for provider in api_key_models):
            if not self.config.get("BREEZE_EMBEDDING_API_KEY"):
                print(f"Error: BREEZE_EMBEDDING_API_KEY is required for model '{embedding_model}'")
    ...
    def create_directories(self):
        ...
        try:
            ...
            subprocess.run(["sudo", "chown", os.environ["USER"], str(log_dir)], check=True)
    ...
    def install_service(self):
        ...
        if subprocess.run(
            ...
            capture_output=True, 
            text=True
        ).stdout.find("com.breeze-mcp.server") != -1:
            print("Unloading existing agent...")
            subprocess.run(
                ["launchctl", "unload", str(self.dest_plist)], 
